...
otidalplan_pi::otidalplan_pi(void *ppimgr) : opencpn_plugin_116(ppimgr) {
...
int otidalplan_pi::Init(void) {
  ...
  if (m_botidalplanShowIcon)
    ...
    m_leftclick_tool_id = InsertPlugInToolSVG(
        _T("otidalplan"), _svg_otidalplan, _svg_otidalplan,
        _svg_otidalplan_toggled, wxITEM_CHECK, _("otidalplan"), _T(""), NULL,
        otidalplan_TOOL_POSITION, 0, this);
#else
    m_leftclick_tool_id = InsertPlugInTool(
        _T(""), _img_otidalplan, _img_otidalplan, wxITEM_CHECK, _("otidalplan"),
        _T(""), NULL, otidalplan_TOOL_POSITION, 0, this);
#endif
...
int otidalplan_pi::GetAPIVersionMajor() { return atoi(API_VERSION); }
...
wxString otidalplan_pi::GetCommonName() { return PLUGIN_API_NAME; }

wxString otidalplan_pi::GetShortDescription() { return PKG_SUMMARY; }

wxString otidalplan_pi::GetLongDescription() { return PKG_DESCRIPTION; }

void otidalplan_pi::SetDefaults(void) {}

int otidalplan_pi::GetToolbarToolCount(void) { return 1; }
...
void otidalplan_pi::OnToolbarToolCallback(int id) {
  if (!m_potidalplanDialog) {
    ...
    m_potidalplanDialog->Move(
        0, 0);  // workaround for gtk autocentre dialog behavior
    m_potidalplanDialog->Move(p);

    // Create the drawing factory
    m_potidalplanOverlayFactory =
        new otidalplanOverlayFactory(*m_potidalplanDialog);
    m_potidalplanOverlayFactory->SetParentSize(m_display_width,
                                               m_display_height);
  }
...
bool otidalplan_pi::RenderOverlay(wxDC &dc, PlugIn_ViewPort *vp) {
  if (!m_potidalplanDialog || !m_potidalplanDialog->IsShown() ||
      !m_potidalplanOverlayFactory)
    return false;
...
bool otidalplan_pi::RenderGLOverlay(wxGLContext *pcontext,
                                    PlugIn_ViewPort *vp) {
  if (!m_potidalplanDialog || !m_potidalplanDialog->IsShown() ||
      !m_potidalplanOverlayFactory)
    return false;
...
void otidalplan_pi::SetCursorLatLon(double lat, double lon) {
  if (m_potidalplanDialog) m_potidalplanDialog->SetCursorLatLon(lat, lon);
